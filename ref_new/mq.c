...
static inline uint32_t mq_half(uint32_t x)
{
    x += Q & -(x & 1);
    return x >> 1;
}

static inline uint32_t mq_mred(uint32_t x)
{
    uint32_t t;
    __asm__("mul %1, %0, %3\n\tumaal %3, %0, %1, %2"
            : "+r"(x), "=&r"(t)
...
static inline uint32_t mq_mmul(uint32_t x, uint32_t y)
{
    return mq_mred(x * y);
}

static uint32_t mq_div(uint32_t x, uint32_t y)
{
...
static inline __m256i mq_add_x16(__m256i x, __m256i y)
{
    ...
    __m256i b = _mm256_add_epi16(
        a, _mm256_and_si256(qq, _mm256_srai_epi16(a, 15)));
...
TARGET_AVX2
static inline __m256i mq_half_x16(__m256i x)
{
    __m256i qq = _mm256_set1_epi16(Q);
...
TARGET_AVX2
static inline __m256i mq_mred_x16(__m256i lo, __m256i hi)
{
    ...
    __m256i x = _mm256_add_epi16(
        _mm256_add_epi16(_mm256_mulhi_epu16(lo, q1ilox16),
...
TARGET_AVX2
static inline __m256i mq_mmul_x16(__m256i x, __m256i y)
{
    return mq_mred_x16(_mm256_mullo_epi16(x, y), _mm256_mulhi_epu16(x, y));
}

TARGET_AVX2
static __m256i mq_div_x16(__m256i x, __m256i y)
{
    /* Convert y to Montgomery representation. */
...
void avx2_mqpoly_signed_to_int(unsigned logn, uint16_t *d)
{
    if (logn >= 4) {
        ...
        for (size_t i = 0; i < (1u << (logn - 4)); i++) {
            ...
            y = _mm256_sub_epi16(_mm256_setzero_si256(), y);
            y = _mm256_add_epi16(
                y, _mm256_and_si256(qq, _mm256_srai_epi16(y, 15)));
...
int avx2_mqpoly_int_to_small(unsigned logn, const uint16_t *d, int8_t *f)
{
    if (logn >= 4) {
        ...
        __m256i sm = _mm256_setr_epi8(
            0, 2, 4, 6, 8, 10, 12, 14, -1, -1, -1, -1, -1, -1, -1, -1, 0,
...
void avx2_mqpoly_ext_to_int(unsigned logn, uint16_t *d)
{
    if (logn >= 4) {
        ...
        for (size_t i = 0; i < (1u << (logn - 4)); i++) {
            ...
            y = _mm256_add_epi16(
                y, _mm256_and_si256(
                       qq, _mm256_cmpeq_epi16(y, _mm256_setzero_si256())));
            _mm256_storeu_si256(dp + i, y);
        }
    } else {
        size_t n = (size_t)1 << logn;
...
void avx2_mqpoly_int_to_ext(unsigned logn, uint16_t *d)
{
    if (logn >= 4) {
        ...
        for (size_t i = 0; i < (1u << (logn - 4)); i++) {
            __m256i y = _mm256_loadu_si256(dp + i);
            y = _mm256_sub_epi16(
                y, _mm256_and_si256(qq, _mm256_cmpeq_epi16(y, qq)));
            _mm256_storeu_si256(dp + i, y);
        }
    } else {
        size_t n = (size_t)1 << logn;
...
#if FNDSA_AVX2
TARGET_AVX2
static inline void avx2_NTT32(__m256i *a0, __m256i *a1, size_t k)
{
    ...
    g1_1 = mq_GM[(k << 1) + 1];
    yg1 =
        _mm256_setr_epi16(g1_0, g1_0, g1_0, g1_0, g1_0, g1_0, g1_0, g1_0,
    ...
    yg2 = _mm256_setr_epi64x(mq_GM[(k << 2) + 0], mq_GM[(k << 2) + 1],
    ...
    yg3 =
        _mm256_cvtepi16_epi32(_mm_loadu_si128((const __m128i *)mq_GM + k));
    ...
    /* t = 2, m = 16 */
    ysk = _mm256_setr_epi8(0, 1, 4, 5, 8, 9, 12, 13, 2, 3, 6, 7, 10, 11,
    ...
    yt2 = mq_mmul_x16(yt2, _mm256_loadu_si256((const __m256i *)mq_GM + k));
    ya0 = mq_add_x16(yt1, yt2);
    ya1 = mq_sub_x16(yt1, yt2);
        ...
        for (unsigned lm = 0; lm < (logn - 5); lm++) {
            ...
            for (size_t i = 0; i < m; i++) {
                ...
                for (size_t j = 0; j < ht; j++) {
                    ...
                    _mm256_storeu_si256(dp + j1, mq_add_x16(y1, y2));
                    _mm256_storeu_si256(dp + j2, mq_sub_x16(y1, y2));
                }
                j0 += t;
            }
            t = ht;
...
#if FNDSA_AVX2
TARGET_AVX2
static inline void avx2_iNTT32(__m256i *a0, __m256i *a1, size_t k)
{
    ...
    yig2 = _mm256_setr_epi64x(mq_iGM[(k << 2) + 0], mq_iGM[(k << 2) + 1],
    ...
    ig1_0 = mq_iGM[(k << 1) + 0];
    ig1_1 = mq_iGM[(k << 1) + 1];
    yig1 = _mm256_setr_epi16(ig1_0, ig1_0, ig1_0, ig1_0, ig1_0, ig1_0,
                             ig1_0, ig1_0, ig1_1, ig1_1, ig1_1, ig1_1,
                             ig1_1, ig1_1, ig1_1, ig1_1);
    ya0 = mq_half_x16(mq_add_x16(yt1, yt2));
        ...
        for (unsigned lm = 5; lm < logn; lm++) {
            ...
            for (size_t i = 0; i < hm; i++) {
                ...
                for (size_t j = 0; j < t; j++) {
                    ...
                    _mm256_storeu_si256(
                        dp + j2, mq_mmul_x16(ys, mq_sub_x16(y1, y2)));
                }
                j0 += dt;
            }
            t = dt;
...
#if FNDSA_AVX2
TARGET_AVX2
void avx2_mqpoly_div_small(unsigned logn, const int8_t *g, const int8_t *f,
                           uint16_t *h, uint16_t *tmp)
{
    avx2_mqpoly_small_to_int(logn, f, tmp);
...
uint32_t avx2_mqpoly_sqnorm_ext(unsigned logn, const uint16_t *a)
{
    if (logn >= 4) {
        ...
        for (size_t i = 0; i < (1u << (logn - 4)); i++) {
            ...
            __m256i y0 =
                _mm256_blend_epi16(ylo, _mm256_bslli_epi128(yhi, 2), 0xAA);
            __m256i y1 =
                _mm256_blend_epi16(_mm256_bsrli_epi128(ylo, 2), yhi, 0xAA);
        ...
        ysat = _mm256_or_si256(ysat, ys);
        __m128i xs = _mm_add_epi32(_mm256_castsi256_si128(ys),
...
{
    static const uint32_t SQBETA[] = {0, /* unused */
                                      101498,   208714,  428865,  892039,
                                      1852696,  3842630, 7959734, 16468416,
                                      34034726, 70265242};
