 ...
 */
static int inner_verify(unsigned logn_min, unsigned logn_max,
                        ...
                        size_t tmp_len)
{
    ...
    hash_to_point(logn, sigbuf + 1, hk, ctx, ctx_len, id, hv, hv_len, t1);
    mqpoly_ext_to_int(logn, t1);
...
#if FNDSA_AVX2
TARGET_AVX2
static int avx2_inner_verify(unsigned logn_min, unsigned logn_max,
                             const void *sig, size_t sig_len,
                             const void *vrfy_key, size_t vrfy_key_len,
...
#if FNDSA_AVX2
    if (has_avx2()) {
        return avx2_inner_verify(9, 10, sig, sig_len, vrfy_key,
                                 vrfy_key_len, ctx, ctx_len, id, hv,
                                 hv_len);
    }
#endif
    uint8_t tmp[4 * 1024 + 31];
    return inner_verify(9, 10, sig, sig_len, vrfy_key, vrfy_key_len, ctx,
                        ctx_len, id, hv, hv_len, tmp, sizeof tmp);
}
...
#if FNDSA_AVX2
    if (has_avx2()) {
        return avx2_inner_verify(2, 8, sig, sig_len, vrfy_key,
                                 vrfy_key_len, ctx, ctx_len, id, hv,
                                 hv_len);
    }
#endif
    uint8_t tmp[4 * 256 + 31];
    return inner_verify(2, 8, sig, sig_len, vrfy_key, vrfy_key_len, ctx,
                        ctx_len, id, hv, hv_len, tmp, sizeof tmp);
}
...
{
    return inner_verify(9, 10, sig, sig_len, vrfy_key, vrfy_key_len, ctx,
                        ctx_len, id, hv, hv_len, tmp, tmp_len);
}
