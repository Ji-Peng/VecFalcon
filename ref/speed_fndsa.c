...
static double bench_sign(unsigned logn, unsigned *x)
{
    ...
    qsort(tt, 1000, sizeof(uint64_t), &cmp_u64);
    *x ^= seed[0] ^ seed[1];
    return (double)tt[500];
}
...
static double bench_verify(unsigned logn, unsigned *x)
{
    ...
    seed[0] ^= 0x01;
    uint64_t tt[1000];
    uint8_t sig[1200][FNDSA_SIGNATURE_SIZE(10)];
    for (int i = 0; i < 1200; i++) {
        fndsa_sign_seeded(sk, FNDSA_SIGN_KEY_SIZE(logn), NULL, 0,
                          FNDSA_HASH_ID_RAW, "test", 4, seed, sizeof seed,
    ...
    for (int i = 0; i < 1200; i++) {
        uint64_t begin = core_cycles();
        int r = fndsa_verify(sig[i], FNDSA_SIGNATURE_SIZE(logn), vk,
