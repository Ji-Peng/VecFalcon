 ...
 * system, it will crash with "illegal instruction" or "segmentation
 * fault". Access to the in-CPU cycle counter must first be allowed, which
 * usually needs an action from the superuser (root) but possibly a kernel
 * module. Operating systems prevent access (by default) to performance
 * counters because such counters may help local attacker exercise timing
 * attacks to extract information from processes that they should not be
 * able to access. On a personal, mono-user machine, allowing access to
 * such counters is much less an issue.
 *
 ...
 *
 * Note that if you run in a virtual machine, then access to the
 * performance counters must probably be enabled on both guest and host.
 *
 * aarch64:
 * ========
 ...
 * idle state. It can then be necessary to disable that state, with
 * something like that (as root, of course): echo 1 >
 * /sys/devices/system/cpu/cpu0/cpuidle/state1/disable echo 1 >
 * /sys/devices/system/cpu/cpu1/cpuidle/state1/disable echo 1 >
 * /sys/devices/system/cpu/cpu2/cpuidle/state1/disable echo 1 >
 * /sys/devices/system/cpu/cpu3/cpuidle/state1/disable and only then load
 * the module which enables userland access to the counter. Disabling the
 * idle state was necessary on an ODROID C4 (ARM Cortex-A55) running
 * Ubuntu 22.04 (kernel 4.9.337-13), but not on a Raspberry Pi 5 (ARM
 * Cortex-A76) running Ubuntu 24.04 (kernel 6.8.0-1015-raspi).
 *
 * RISC-V:
 * =======
...
#    endif
// clang-format off
static inline uint64_t core_cycles(void)
{
    uint64_t result;
    __asm__
    volatile("rdtsc; shlq $32,%%rdx; orq %%rdx,%%rax"
        : "=a"(result) : : "%rdx");
    return result;
}
// clang-format on
...
static double bench_keygen(unsigned logn, unsigned *x)
{
...
static double bench_sign(unsigned logn, unsigned *x)
{
    uint64_t z = core_cycles();
    uint8_t seed[8];
    for (int i = 0; i < 8; i++) {
    ...
    for (int i = 0; i < WARMUP_N; i++) {
        fndsa_sign_seeded(sk, FNDSA_SIGN_KEY_SIZE(logn), NULL, 0,
    ...
    qsort(tt, TESTS_N, sizeof(uint64_t), &cmp_u64);
    *x ^= seed[0] ^ seed[1];
    return (double)tt[TESTS_N / 2];
}
...
static double bench_verify(unsigned logn, unsigned *x)
{
    ...
    fndsa_sign_seeded(sk, FNDSA_SIGN_KEY_SIZE(logn), NULL, 0,
                      FNDSA_HASH_ID_RAW, "test", 4, seed, sizeof seed, sig,
                      FNDSA_SIGNATURE_SIZE(logn));
    int r = fndsa_verify(sig, FNDSA_SIGNATURE_SIZE(logn), vk,
                         FNDSA_VRFY_KEY_SIZE(logn), NULL, 0,
                         FNDSA_HASH_ID_RAW, msg, 4);
    if (r != 1) {
        printf("Verification failed!\n");
        return 0;
    }
    for (int i = 0; i < WARMUP_N; i++) {
        fndsa_verify(sig, FNDSA_SIGNATURE_SIZE(logn), vk,
...
static double bench_sign_core_512()
{
    uint8_t sig[FNDSA_SIGNATURE_SIZE(9)];
    uint8_t seed[8];
    uint8_t tmp[(512) * 78 + 31];
    uint64_t tt[TESTS_N];
    ...
    for (int i = 0; i < WARMUP_N; i++) {
        sign_core(9, KAT512_f, KAT512_g, KAT512_F, KAT512_G,
                  KAT512_hashed_vk, NULL, 0, FNDSA_HASH_ID_RAW, KAT512_hv,
                  4, seed, sizeof(seed), sig, tmp);
    }
    for (int i = 0; i < TESTS_N; i++) {
        uint64_t begin = core_cycles();
        sign_core(9, KAT512_f, KAT512_g, KAT512_F, KAT512_G,
...
static double bench_sign_core_1024()
{
    uint8_t sig[FNDSA_SIGNATURE_SIZE(10)];
    uint8_t seed[8];
    ...
    for (int i = 0; i < WARMUP_N; i++) {
        sign_core(10, KAT1024_f, KAT1024_g, KAT1024_F, KAT1024_G,
...
#if FNDSA_SHAKE256X4 == 1
    printf("SHAKE256X4 variant\n");
#endif
    printf("n = 512:\n");
    printf("%-30s %13.0f\n", "FN-DSA sign:", bench_sign(9, &x));
    printf("%-30s %13.0f\n", "FN-DSA sign_core:", bench_sign_core_512());
    printf("%-30s %13.0f\n", "FN-DSA verify:", bench_verify(9, &x));
