...
/* Make a value out of the sign bit s, exponent e, and mantissa.
   ...
   If the value is a zero, then it must be specified as:
      m = 0
      e = -1076  */
static inline fpr make(uint64_t s, int32_t e, uint64_t m)
{
    uint64_t cc = (0xC8u >> ((unsigned)m & 7)) & 1;
    return (s << 63) + ((uint64_t)(uint32_t)(e + 1076) << 52) + (m >> 2) +
           cc;
}

/* Like make(), but it also sets e to -1076 in case m = 0. m MUST be either
   0 or in the [2^54,2^55-1] range. */
static inline fpr make_z(uint64_t s, int32_t e, uint64_t m)
{
...
static inline uint32_t lzcnt64_nonzero(uint64_t x)
{
...
fpr fpr_add(fpr x, fpr y)
{
    ...
    ex &= 0x7FF;
    uint64_t xu =
        ((x & M52) << 3) | ((uint64_t)((ex + 0x7FF) >> 11) << 55);
    ex -= 1078;
    ...
    ey &= 0x7FF;
    uint64_t yu =
        ((y & M52) << 3) | ((uint64_t)((ey + 0x7FF) >> 11) << 55);
    ey -= 1078;
