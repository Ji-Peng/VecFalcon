 ...
 */
int fndsa_keygen_seeded_temp(unsigned logn, const void *seed,
                             size_t seed_len, void *sign_key,
                             void *vrfy_key, void *tmp, size_t tmp_len);

/*
 * Sign a message.
 *    sign_key, sign_key_len   signing key (encoded)
 *    ctx, ctx_len             domain separation context (at most 255
 * bytes) id                       identifier for the pre-hash function hv,
 * hv_len               pre-hashed message (or raw message) sig,
 * sig_max_len         output buffer for the signature This function
 * returns 0 on error; error conditions include: signing key cannot be
 * decoded; signing key is for a weak degree (i.e. not 512 nor 1024);
 * system random generator failed to provide the requested random bytes;
 * output buffer is not large enough for the signature.
 *
 * If sig is NULL, then the signature is not generated, but the signing key
 * is still decoded; returned value is then the signature length (in
 * bytes), or 0 on error (signing key cannot be decoded or uses a weak
 * degree). If sig is not NULL, then sig_max_len should be the maximum
 * buffer size for the signature; an error (0) is returned if that length
 * is lower than the signature size implied by the degree of the signing
 * key. If the output buffer is large enough, and the signature generation
 * succeeds (i.e. the system random generator does not fail), then the
 * signature is written in sig[] and the signature length (in bytes) is
 * returned.
 */
size_t fndsa_sign(const void *sign_key, size_t sign_key_len,
                  const void *ctx, size_t ctx_len, const char *id,
                  const void *hv, size_t hv_len, void *sig,
                  size_t max_sig_len);
 ...
 * length seed_len bytes). It is the responsibility of the caller to
 * provide a seed with sufficient entropy (preferably 320 bits or more,
 * i.e. a seed of at least 40 bytes). This function is provided mostly for
 * testing purposes, and for bare-metal systems with no OS-provided random
 * generator.
 */
size_t fndsa_sign_seeded(const void *sign_key, size_t sign_key_len,
                         const void *ctx, size_t ctx_len, const char *id,
                         const void *hv, size_t hv_len, const void *seed,
                         size_t seed_len, void *sig, size_t max_sig_len);
...
size_t fndsa_sign_temp(const void *sign_key, size_t sign_key_len,
                       const void *ctx, size_t ctx_len, const char *id,
                       const void *hv, size_t hv_len, void *sig,
                       size_t max_sig_len, void *tmp, size_t tmp_len);

/*
 * Similar to fndsa_sign_seeded(), but with an explicitly provided
 * temporary area as in fndsa_sign_temp().
 */
size_t fndsa_sign_seeded_temp(const void *sign_key, size_t sign_key_len,
                              const void *ctx, size_t ctx_len,
                              const char *id, const void *hv,
                              size_t hv_len, const void *seed,
                              size_t seed_len, void *sig,
                              size_t max_sig_len, void *tmp,
                              size_t tmp_len);
...
size_t fndsa_sign_weak_temp(const void *sign_key, size_t sign_key_len,
                            ...
                            void *sig, size_t max_sig_len, void *tmp,
                            size_t tmp_len);
size_t fndsa_sign_weak_seeded_temp(const void *sign_key,
                                   size_t sign_key_len, const void *ctx,
                                   size_t ctx_len, const char *id,
 ...
 *    ctx, ctx_len             domain separation context (at most 255
 * bytes) id                       identifier for the pre-hash function hv,
 * hv_len               pre-hashed message (or raw message) Returned value
 * is 1 if the signature is valid, 0 otherwise. Decoding errors on the
 * verifying key also imply a returned value of 0.
 *
 * This function accepts only the standard degrees (512 or 1024).
 */
int fndsa_verify(const void *sig, size_t sig_len, const void *vrfy_key,
                 size_t vrfy_key_len, const void *ctx, size_t ctx_len,
                 const char *id, const void *hv, size_t hv_len);
...
int fndsa_verify_temp(const void *sig, size_t sig_len,
                      const void *vrfy_key, size_t vrfy_key_len,
                      const void *ctx, size_t ctx_len, const char *id,
                      const void *hv, size_t hv_len, void *tmp,
                      size_t tmp_len);
