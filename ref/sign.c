...
static size_t sign_step1(unsigned logn, const uint8_t *sign_key,
                         ...
                         uint8_t *sig, void *tmp)
{
    ...
    return sign_core(logn, f, g, F, G, hashed_key, ctx, ctx_len, id, hv,
                     hv_len, seed, seed_len, sig, tmp);
 ...
 */
#define SIGN_WRAP(sz)                                                     \
    static size_t sign_##sz(                                              \
        unsigned logn, const uint8_t *sign_key, const uint8_t *ctx,       \
        size_t ctx_len, const char *id, const uint8_t *hv, size_t hv_len, \
        const uint8_t *seed, size_t seed_len, uint8_t *sig)               \
    {                                                                     \
        uint8_t tmp[(sz) * 78 + 31];                                      \
        return sign_step1(logn, sign_key, ctx, ctx_len, id, hv, hv_len,   \
...
static size_t sign_wrapper(int weak, const uint8_t *sign_key,
                           size_t sign_key_len, const uint8_t *ctx,
                           size_t ctx_len, const char *id,
                           const uint8_t *hv, size_t hv_len,
                           const uint8_t *seed, size_t seed_len,
                           uint8_t *sig, size_t max_sig_len, void *tmp,
                           size_t tmp_len)
{
    ...
    if (tmp == NULL) {
        switch (logn) {
            case 6:
                return sign_64(logn, sign_key, ctx, ctx_len, id, hv,
                               hv_len, seed, seed_len, sig);
            case 7:
                return sign_128(logn, sign_key, ctx, ctx_len, id, hv,
                                hv_len, seed, seed_len, sig);
            case 8:
                return sign_256(logn, sign_key, ctx, ctx_len, id, hv,
                                hv_len, seed, seed_len, sig);
            case 9:
                return sign_512(logn, sign_key, ctx, ctx_len, id, hv,
                                hv_len, seed, seed_len, sig);
            case 10:
                return sign_1024(logn, sign_key, ctx, ctx_len, id, hv,
                                 hv_len, seed, seed_len, sig);
            default:
                return sign_32(logn, sign_key, ctx, ctx_len, id, hv,
                               hv_len, seed, seed_len, sig);
        }
    } else {
...
size_t fndsa_sign(const void *sign_key, size_t sign_key_len,
                  ...
                  size_t max_sig_len)
{
    return sign_wrapper(0, sign_key, sign_key_len, ctx, ctx_len, id, hv,
                        hv_len, NULL, 0, sig, max_sig_len, NULL, 0);
}
...
{
    return sign_wrapper(0, sign_key, sign_key_len, ctx, ctx_len, id, hv,
                        hv_len, seed, seed_len, sig, max_sig_len, NULL, 0);
}
...
{
    return sign_wrapper(0, sign_key, sign_key_len, ctx, ctx_len, id, hv,
                        hv_len, NULL, 0, sig, max_sig_len, tmp, tmp_len);
}
...
{
    return sign_wrapper(0, sign_key, sign_key_len, ctx, ctx_len, id, hv,
                        hv_len, seed, seed_len, sig, max_sig_len, tmp,
                        tmp_len);
}
...
size_t fndsa_sign_weak(const void *sign_key, size_t sign_key_len,
                       ...
                       size_t max_sig_len)
{
    return sign_wrapper(1, sign_key, sign_key_len, ctx, ctx_len, id, hv,
                        hv_len, NULL, 0, sig, max_sig_len, NULL, 0);
}
