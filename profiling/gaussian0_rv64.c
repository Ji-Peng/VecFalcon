...
void gaussian0_1w(sampler_state *ss, int32_t *z_bimodal, int32_t *z_square)
{
    uint64_t in_72b[2];
    /* Get a random 72-bit value, into three 24-bit limbs (v0..v2). */
    in_72b[0] = prng_next_u64(&ss->pc);
    in_72b[1] = prng_next_u8(&ss->pc);

    int32_t z;
    gaussian0_rv64im(&z, in_72b);
...
void test_gaussian0()
{
    ...
    for (i = 0; i < SAMPLES_N; i += 1) {
        gaussian0_ref(&ss0, &z0_bi.coeffs[i], &z0_sq.coeffs[i]);
    }
    ...
    for (i = 0; i < SAMPLES_N; i += 16) {
        gaussian0_ref_16w(&ss0, &z0_bi.coeffs[i], &z0_sq.coeffs[i]);
    }
    memset(&z1_bi, 0, sizeof(z1_bi));
    memset(&z1_sq, 0, sizeof(z1_sq));
    sampler_init(&ss1, 9, seed, 32);
    ...
    for (i = 0; i < SAMPLES_N; i += 64) {
        gaussian0_ref_64w(&ss0, &z0_bi.coeffs[i], &z0_sq.coeffs[i]);
    }
    memset(&z1_bi, 0, sizeof(z1_bi));
    memset(&z1_sq, 0, sizeof(z1_sq));
    sampler_init(&ss1, 9, seed, 32);
