...
Team website: https://keccak.team/
...
This implementation comes with KeccakP-1600-times8-SnP.h in the same
folder. Please refer to LowLevel.build for the exact list of other files it
...
/* on Mac OS-X and possibly others, ALIGN(x) is defined in param.h, and
 * -Werror chokes on the redef. */
#ifdef ALIGN
#    undef ALIGN
#endif
...
#define KeccakP1600times8_implementation_config \
    "AVX512, all rounds unrolled"
#define KeccakP1600times8_fullUnrolling
#define KeccakP1600times8_unrolling 0
#define KeccakP1600times8_useAVX512

#define KeccakP1600times8_implementation \
    "512-bit SIMD implementation "       \
    "(" KeccakP1600times8_implementation_config ")"
...
#define KeccakP1600times8_AddByte(states, instanceIndex, byte, offset)    \
    ((unsigned char *)(states))[(instanceIndex) * 8 +                     \
                                ((offset) / 8) * 8 * 8 + (offset) % 8] ^= \
        (byte)
void KeccakP1600times8_AddBytes(void *states, unsigned int instanceIndex,
                                const unsigned char *data,
                                unsigned int offset, unsigned int length);
void KeccakP1600times8_AddLanesAll(void *states, const unsigned char *data,
                                   unsigned int laneCount,
                                   unsigned int laneOffset);
void KeccakP1600times8_OverwriteBytes(void *states,
                                      unsigned int instanceIndex,
                                      const unsigned char *data,
                                      unsigned int offset,
                                      unsigned int length);
void KeccakP1600times8_OverwriteLanesAll(void *states,
                                         const unsigned char *data,
                                         unsigned int laneCount,
                                         unsigned int laneOffset);
void KeccakP1600times8_OverwriteWithZeroes(void *states,
                                           unsigned int instanceIndex,
...
void KeccakP1600times8_ExtractBytes(const void *states,
                                    unsigned int instanceIndex,
                                    unsigned char *data,
                                    unsigned int offset,
                                    unsigned int length);
void KeccakP1600times8_ExtractLanesAll(const void *states,
                                       unsigned char *data,
                                       unsigned int laneCount,
                                       unsigned int laneOffset);
void KeccakP1600times8_ExtractAndAddBytes(const void *states,
                                          unsigned int instanceIndex,
                                          const unsigned char *input,
                                          unsigned char *output,
                                          unsigned int offset,
                                          unsigned int length);
void KeccakP1600times8_ExtractAndAddLanesAll(const void *states,
                                             const unsigned char *input,
                                             unsigned char *output,
                                             unsigned int laneCount,
                                             unsigned int laneOffset);
size_t KeccakF1600times8_FastLoop_Absorb(void *states,
                                         unsigned int laneCount,
...
size_t KeccakP1600times8_12rounds_FastLoop_Absorb(
    ...
    size_t dataByteLen);
size_t KeccakP1600times8_KravatteCompress(uint64_t *xAccu, uint64_t *kRoll,
                                          const unsigned char *input,
                                          size_t inputByteLen);
size_t KeccakP1600times8_KravatteExpand(uint64_t *yAccu,
                                        const uint64_t *kRoll,
                                        unsigned char *output,
                                        size_t outputByteLen);
void KeccakP1600times8_KT128ProcessLeaves(const unsigned char *input,
                                          unsigned char *output);
void KeccakP1600times8_KT256ProcessLeaves(const unsigned char *input,
                                          unsigned char *output);

/*
** Uncomment the define hereunder when compiling for a CPU without AVX-512
SIMD. #define SIMULATE_AVX512
*/
...
#    define LOAD8_32(a, b, c, d, e, f, g, h)                          \
        _mm256_set_epi32((uint64_t)(a), (uint32_t)(b), (uint32_t)(c), \
                         (uint32_t)(d), (uint32_t)(e), (uint32_t)(f), \
                         (uint32_t)(g), (uint32_t)(h))
#    define LOAD8_64(a, b, c, d, e, f, g, h)                          \
        _mm512_set_epi64((uint64_t)(a), (uint64_t)(b), (uint64_t)(c), \
                         (uint64_t)(d), (uint64_t)(e), (uint64_t)(f), \
                         (uint64_t)(g), (uint64_t)(h))
#    define LOAD_GATHER8_64(idx, p) \
        _mm512_i32gather_epi64(idx, (const void *)(p), 8)

#    define STORE_SCATTER8_64(p, idx, v) \
        _mm512_i32scatter_epi64((void *)(p), idx, v, 8)
...
#define Add_In(argIndex)            \
    stateAsLanes[argIndex] =        \
        XOR(stateAsLanes[argIndex], \
            LOAD_GATHER8_64(index, dataAsLanes + argIndex))
    index = LOAD8_32(7 * laneOffset, 6 * laneOffset, 5 * laneOffset,
                     4 * laneOffset, 3 * laneOffset, 2 * laneOffset,
                     1 * laneOffset, 0 * laneOffset);
    if (laneCount >= 16) {
        Add_In(0);
        Add_In(1);
        Add_In(2);
                                      ...
                                      unsigned int length)
{
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        if (bytesInLane > sizeLeft)
            bytesInLane = sizeLeft;
        memcpy(((unsigned char *)&statesAsLanes[laneIndex(instanceIndex,
                                                          lanePosition)]) +
                   offsetInLane,
               curData, bytesInLane);
        sizeLeft -= bytesInLane;
...
#define OverWr(argIndex) \
    stateAsLanes[argIndex] = LOAD_GATHER8_64(index, dataAsLanes + argIndex)
    index = LOAD8_32(7 * laneOffset, 6 * laneOffset, 5 * laneOffset,
                     4 * laneOffset, 3 * laneOffset, 2 * laneOffset,
                     1 * laneOffset, 0 * laneOffset);
    if (laneCount >= 16) {
                                           ...
                                           unsigned int byteCount)
{
    ...
    if (sizeLeft > 0) {
        memset(&statesAsLanes[laneIndex(instanceIndex, lanePosition)], 0,
               sizeLeft);
    }
}
                                    ...
                                    unsigned int length)
{
    ...
    if ((sizeLeft > 0) && (offsetInLane != 0)) {
        ...
        if (bytesInLane > sizeLeft)
            bytesInLane = sizeLeft;
        memcpy(curData,
               ((unsigned char *)&statesAsLanes[laneIndex(instanceIndex,
                                                          lanePosition)]) +
                   offsetInLane,
               bytesInLane);
        sizeLeft -= bytesInLane;
...
#define Extr(argIndex)                               \
    STORE_SCATTER8_64(dataAsLanes + argIndex, index, \
                      stateAsLanes[argIndex])
    index = LOAD8_32(7 * laneOffset, 6 * laneOffset, 5 * laneOffset,
                     4 * laneOffset, 3 * laneOffset, 2 * laneOffset,
                     1 * laneOffset, 0 * laneOffset);
...
#define ExtrAdd(argIndex)                           \
    STORE_SCATTER8_64(outAsLanes + argIndex, index, \
                      XOR(stateAsLanes[argIndex],   \
                          LOAD_GATHER8_64(index, inAsLanes + argIndex)))
    index = LOAD8_32(7 * laneOffset, 6 * laneOffset, 5 * laneOffset,
                     4 * laneOffset, 3 * laneOffset, 2 * laneOffset,
                     1 * laneOffset, 0 * laneOffset);
    if (laneCount >= 16) {
...
static ALIGN(KeccakP1600times8_statesAlignment) const uint64_t
    KeccakP1600RoundConstants[24] = {
...
#define KeccakP_2rounds(i)                                               \
    KeccakP_ThetaRhoPiChiIota0(_ba, _ke, _si, _go, _mu,                  \
                               CONST8_64(KeccakP1600RoundConstants[i])); \
    KeccakP_ThetaRhoPiChi1(_ma, _be, _ki, _so, _gu);                     \
    KeccakP_ThetaRhoPiChi2(_ga, _me, _bi, _ko, _su);                     \
    KeccakP_ThetaRhoPiChi3(_sa, _ge, _mi, _bo, _ku);                     \
...
#define rounds4 KeccakP_4rounds(20)
                                         ...
                                         size_t dataByteLen)
{
    ...
    if (laneCount == 21) {
        ...
        copyFromState(statesAsLanes);
        index = LOAD8_32(7 * laneOffsetParallel, 6 * laneOffsetParallel,
                         5 * laneOffsetParallel, 4 * laneOffsetParallel,
                         3 * laneOffsetParallel, 2 * laneOffsetParallel,
                         1 * laneOffsetParallel, 0 * laneOffsetParallel);
        while (dataByteLen >= dataMinimumSize) {
#define Add_In(argLane, argIndex) \
    argLane = XOR(argLane, LOAD_GATHER8_64(index, dataAsLanes + argIndex))
            Add_In(_ba, 0);
            Add_In(_be, 1);
            Add_In(_bi, 2);
            Add_In(_bo, 3);
...
#undef Add_In
        ...
        while (dataByteLen >= dataMinimumSize) {
            KeccakP1600times8_AddLanesAll(states, data, laneCount,
                                          laneOffsetParallel);
...
#define XORdata16(X, index, dataAsLanes)                \
    LoadAndTranspose8(dataAsLanes, 0) XOReq(X##ba, r0); \
    XOReq(X##be, r1);                                   \
    XOReq(X##bi, r2);                                   \
    XOReq(X##bo, r3);                                   \
    XOReq(X##bu, r4);                                   \
    ...
    XOReq(X##ge, r6);                                   \
    XOReq(X##gi, r7);                                   \
    LoadAndTranspose8(dataAsLanes, 8) XOReq(X##go, r0); \
    XOReq(X##gu, r1);                                   \
    XOReq(X##ka, r2);                                   \
    XOReq(X##ke, r3);                                   \
    XOReq(X##ki, r4);                                   \
...
void KeccakP1600times8_K12ProcessLeaves(const unsigned char *input,
                                        unsigned char *output)
{
    KeccakP_DeclareVars;
    unsigned int j;
    ...
    index = LOAD8_32(7 * (chunkSize / 8), 6 * (chunkSize / 8),
                     5 * (chunkSize / 8), 4 * (chunkSize / 8),
                     3 * (chunkSize / 8), 2 * (chunkSize / 8),
                     1 * (chunkSize / 8), 0 * (chunkSize / 8));
    for (j = 0; j < (chunkSize - rateInBytes); j += rateInBytes) {
        XORdata21(_, index, (const uint64_t *)input);
        rounds12 input += rateInBytes;
    }
    ...
    rounds12
        ...
        index = LOAD8_32(7 * 4, 6 * 4, 5 * 4, 4 * 4, 3 * 4, 2 * 4, 1 * 4,
                         0 * 4);
...
#define LoadInput(argIndex)        \
    _mm512_i32gather_epi64(gather, \
                           (const long long int *)&in64[argIndex], 8)
#define AddInput(argIndex) \
    XOR(LoadInput(argIndex), CONST8_64(kRoll[argIndex]))

ALIGN(64)
static const uint64_t oLow256[] = {0, 1, 2, 3, 8 + 0, 8 + 1, 8 + 2, 8 + 3};
ALIGN(64)
static const uint64_t oHigh256[] = {4,     5,     6,     7,
                                    8 + 4, 8 + 5, 8 + 6, 8 + 7};

ALIGN(64)
static const uint64_t oLow128[] = {0, 1, 8 + 0, 8 + 1, 4, 5, 8 + 4, 8 + 5};
ALIGN(64)
static const uint64_t oHigh128[] = {2, 3, 8 + 2, 8 + 3,
                                    6, 7, 8 + 6, 8 + 7};

ALIGN(64)
static const uint64_t oLow64[] = {0, 8 + 0, 2, 8 + 2, 4, 8 + 4, 6, 8 + 6};
ALIGN(64)
static const uint64_t oHigh64[] = {1, 8 + 1, 3, 8 + 3, 5, 8 + 5, 7, 8 + 7};

ALIGN(64)
static const uint64_t o01234_012[] = {0, 1, 2, 3, 4, 8 + 0, 8 + 1, 8 + 2};
ALIGN(64)
static const uint64_t o1234_0123[] = {1,     2,     3,     4,
                                      8 + 0, 8 + 1, 8 + 2, 8 + 3};
ALIGN(64)
static const uint64_t o1234567_0[] = {1, 2, 3, 4, 5, 6, 7, 8 + 0};
ALIGN(64)
static const uint64_t o1234567_3[] = {1, 2, 3, 4, 5, 6, 7, 8 + 3};
ALIGN(64)
static const uint64_t o1234567_4[] = {1, 2, 3, 4, 5, 6, 7, 8 + 4};
ALIGN(64)
static const uint64_t o234567_45[] = {2, 3, 4, 5, 6, 7, 8 + 4, 8 + 5};
ALIGN(64)
static const uint64_t o34567_456[] = {3, 4, 5, 6, 7, 8 + 4, 8 + 5, 8 + 6};

ALIGN(32)
static const uint32_t oGatherScatter[] = {0 * 25, 1 * 25, 2 * 25, 3 * 25,
                                          4 * 25, 5 * 25, 6 * 25, 7 * 25};

#if defined(__i386__) || defined(_M_IX86)
#    define _mm256_extract_epi64(a, index)                   \
        ((uint64_t)_mm256_extract_epi32((a), (index) * 2) || \
         ((uint64_t)_mm256_extract_epi32((a), (index) * 2 + 1) << 32))
#endif
                                          ...
                                          size_t inputByteLen)
{
    ...
    x01234567 =
        _mm512_maskz_loadu_epi64(0x1F, &kRoll[20]);   /* 5 lanes ok */
    _ba = _mm512_maskz_loadu_epi64(0x0F, &kRoll[21]); /* 4 lanes ok */
    _be = XOR3(ROL(x01234567, 7), _ba, _mm512_srli_epi64(_ba, 3));
    x01234567 =
        _mm512_permutex2var_epi64(x01234567, *(V512 *)o01234_012, _be);
    x12345678 =
        _mm512_permutex2var_epi64(x01234567, *(V512 *)o1234_0123, _be);
    ...
    do {
        ...
        Isa = x01234567;
        Ise = x12345678;
        Isu = XOR3(ROL(x01234567, 7), x12345678,
                   _mm512_srli_epi64(x12345678, 3));
        Ise =
            _mm512_permutex2var_epi64(x01234567, *(V512 *)o1234567_3, Isu);
        ...
        x12345678 = _mm512_permutex2var_epi64(
            x01234567, *(V512 *)o1234567_4, x01234567);
    ...
    p1 = *(V512 *)oLow256;
    p2 = *(V512 *)oHigh256;
    Xba = XOR(_mm512_permutex2var_epi64(Xba, p1, Xbu),
              _mm512_permutex2var_epi64(Xba, p2, Xbu));
    Xbe = XOR(_mm512_permutex2var_epi64(Xbe, p1, Xga),
              _mm512_permutex2var_epi64(Xbe, p2, Xga));
    Xbi = XOR(_mm512_permutex2var_epi64(Xbi, p1, Xge),
              _mm512_permutex2var_epi64(Xbi, p2, Xge));
    Xbo = XOR(_mm512_permutex2var_epi64(Xbo, p1, Xgi),
              _mm512_permutex2var_epi64(Xbo, p2, Xgi));

    /* Add horizontally Xgo ... Xma Reduce from lanes 8 to 4 */
    Xgo = XOR(_mm512_permutex2var_epi64(Xgo, p1, Xki),
              _mm512_permutex2var_epi64(Xgo, p2, Xki));
    Xgu = XOR(_mm512_permutex2var_epi64(Xgu, p1, Xko),
              _mm512_permutex2var_epi64(Xgu, p2, Xko));
    Xka = XOR(_mm512_permutex2var_epi64(Xka, p1, Xku),
              _mm512_permutex2var_epi64(Xka, p2, Xku));
    Xke = XOR(_mm512_permutex2var_epi64(Xke, p1, Xma),
              _mm512_permutex2var_epi64(Xke, p2, Xma));

    /* Add horizontally Xme ... Xso Reduce from lanes 8 to 4 */
    Xme = XOR(_mm512_permutex2var_epi64(Xme, p1, Xsa),
              _mm512_permutex2var_epi64(Xme, p2, Xsa));
    Xmi = XOR(_mm512_permutex2var_epi64(Xmi, p1, Xse),
              _mm512_permutex2var_epi64(Xmi, p2, Xse));
    Xmo = XOR(_mm512_permutex2var_epi64(Xmo, p1, Xsi),
              _mm512_permutex2var_epi64(Xmo, p2, Xsi));
    Xmu = XOR(_mm512_permutex2var_epi64(Xmu, p1, Xso),
              _mm512_permutex2var_epi64(Xmu, p2, Xso));

    /* Add horizontally Xba ... Xbo Reduce from lanes 4 to 2 */
    p1 = *(V512 *)oLow128;
    p2 = *(V512 *)oHigh128;
    Xba = XOR(_mm512_permutex2var_epi64(Xba, p1, Xbi),
              _mm512_permutex2var_epi64(Xba, p2, Xbi));
    Xbe = XOR(_mm512_permutex2var_epi64(Xbe, p1, Xbo),
              _mm512_permutex2var_epi64(Xbe, p2, Xbo));

    /* Add horizontally Xgo ... Xke Reduce from lanes 4 to 2 */
    Xgo = XOR(_mm512_permutex2var_epi64(Xgo, p1, Xka),
              _mm512_permutex2var_epi64(Xgo, p2, Xka));
    Xgu = XOR(_mm512_permutex2var_epi64(Xgu, p1, Xke),
              _mm512_permutex2var_epi64(Xgu, p2, Xke));

    /* Add horizontally Xme ... Xmu Reduce from lanes 4 to 2 */
    Xme = XOR(_mm512_permutex2var_epi64(Xme, p1, Xmo),
              _mm512_permutex2var_epi64(Xme, p2, Xmo));
    Xmi = XOR(_mm512_permutex2var_epi64(Xmi, p1, Xmu),
              _mm512_permutex2var_epi64(Xmi, p2, Xmu));

    /* Add horizontally Xba ... Xbe Reduce from lanes 2 to 1 */
    p1 = *(V512 *)oLow64;
    p2 = *(V512 *)oHigh64;
    Xba = XOR(_mm512_permutex2var_epi64(Xba, p1, Xbe),
              _mm512_permutex2var_epi64(Xba, p2, Xbe));

    /* Add horizontally Xgo ... Xgu Reduce from lanes 2 to 1 */
    Xgo = XOR(_mm512_permutex2var_epi64(Xgo, p1, Xgu),
              _mm512_permutex2var_epi64(Xgo, p2, Xgu));

    /* Add horizontally Xme ... Xmi Reduce from lanes 2 to 1 */
    Xme = XOR(_mm512_permutex2var_epi64(Xme, p1, Xmi),
              _mm512_permutex2var_epi64(Xme, p2, Xmi));
    ...
    v1 = _mm256_xor_si256(_mm512_extracti64x4_epi64(Xsu, 0),
                          _mm512_extracti64x4_epi64(Xsu, 1));
...
ALIGN(64)
static const uint64_t o1234567_6[] = {1, 2, 3, 4, 5, 6, 7, 8 + 6};
ALIGN(64)
static const uint64_t o234567_01[] = {2, 3, 4, 5, 6, 7, 8 + 0, 8 + 1};
ALIGN(64)
static const uint64_t o34567_012[] = {3, 4, 5, 6, 7, 8 + 0, 8 + 1, 8 + 2};
ALIGN(64)
static const uint64_t o4567_0123[] = {4,     5,     6,     7,
                                      8 + 0, 8 + 1, 8 + 2, 8 + 3};
ALIGN(64)
static const uint64_t o567_01234[] = {5,     6,     7,     8 + 0,
                                      8 + 1, 8 + 2, 8 + 3, 8 + 4};
ALIGN(64)
static const uint64_t o67_012345[] = {6,     7,     8 + 0, 8 + 1,
                                      8 + 2, 8 + 3, 8 + 4, 8 + 5};
ALIGN(64)
static const uint64_t o7_0123456[] = {7,     8 + 0, 8 + 1, 8 + 2,
                                      8 + 3, 8 + 4, 8 + 5, 8 + 6};
                                        ...
                                        size_t outputByteLen)
{
    ...
    do {
        ...
        /*  roll-e */
        Ima = x01234567;
        Ime = _mm512_permutex2var_epi64(x01234567, *(V512 *)o1234567_6,
                                        x23456789);
        Imi = x23456789;

        x23456789 = XOR3(ROL(Ima, 7), ROL(Ime, 18),
                         _mm512_and_si512(Imi, _mm512_srli_epi64(Ime, 1)));
        Imo =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o1234567_0, x23456789);
        Imu =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o234567_01, x23456789);
        Isa =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o34567_012, x23456789);
        Ise =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o4567_0123, x23456789);
        Isi =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o567_01234, x23456789);
        Iso =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o67_012345, x23456789);
        Isu =
            _mm512_permutex2var_epi64(Imi, *(V512 *)o7_0123456, x23456789);
        x01234567 = Iso;
        Dump("After roll-e", I);
