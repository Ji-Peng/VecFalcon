...
/* Given f, g, F and G, return the basis [[g, -f], [G, -F]] in FFT
   ...
   destination). */
static void basis_to_FFT(unsigned logn, const int8_t *f, const int8_t *g,
                         const int8_t *F, const int8_t *G, fpr *dst)
{
...
/* see sign_inner.h */
TARGET_SSE2 TARGET_NEON size_t
sign_core(unsigned logn, const int8_t *f, const int8_t *g, const int8_t *F,
          const int8_t *G, const uint8_t *hashed_vk, const uint8_t *ctx,
          ...
          const uint8_t *seed, size_t seed_len, uint8_t *sig, void *tmp)
{
    ...
    for (uint32_t counter = 0;; counter++) {
        ...
        hash_to_point(logn, nonce, hashed_vk, ctx, ctx_len, id, hv, hv_len,
                      hm);
        ...
        for (size_t i = 0; i < n; i += 2) {
            ...
            uint16_t zu0 = hm[i + 0] - (uint16_t)_mm_cvtsi128_si32(zt);
            uint16_t zu1 = hm[i + 1] - (uint16_t)_mm_cvtsi128_si32(
        ...
        for (size_t i = 0; i < n; i += 2) {
            ...
            uint16_t zu1 =
                -(uint16_t)_mm_cvtsi128_si32(_mm_bsrli_si128(zt, 4));
            int32_t z0 = (int32_t)*(int16_t *)&zu0;
            int32_t z1 = (int32_t)*(int16_t *)&zu1;
            sqn += (uint32_t)(z0 * z0);
            ng |= sqn;
        ...
        for (size_t i = 0; i < n; i += 2) {
            ...
            uint16_t zu0 = hm[i + 0] - (uint16_t)vgetq_lane_s64(zt, 0);
            uint16_t zu1 = hm[i + 1] - (uint16_t)vgetq_lane_s64(zt, 1);
            int32_t z0 = (int32_t)*(int16_t *)&zu0;
            int32_t z1 = (int32_t)*(int16_t *)&zu1;
            sqn += (uint32_t)(z0 * z0);
            ng |= sqn;
