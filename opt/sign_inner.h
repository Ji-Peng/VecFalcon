 ...
 *   (-1)^s * (2^52 + m) * 2^(e-1075)
 *
 * Values with e = 2047 denote an infinite (m = 0) or a NaN (m != 0);
...
/* Right-shift a 64-bit unsigned value by a possibly secret shift count.
   ...
   On ARM Cortex-M4, compilers apply a branchless sequence of 8
   instructions which leverages the fact that the shift opcodes actually
   use an 8-bit count (and not 5-bit). */
...
static inline int64_t fpr_rint(fpr x)
{
    ...
    uint64_t m =
        ((x << 10) | ((uint64_t)1 << 62)) & (((uint64_t)1 << 63) - 1);
...
#    define FPR_ADD_SUB(a, b, x, y)                                      \
        do {                                                             \
            ...
            __asm__("bl	fndsa_fpr_add_sub"                               \
                    ...
                    :                                                    \
                    : "r4", "r5", "r6", "r7", "r8", "r10", "r11", "r12", \
                      "r14", "s15", "cc");                               \
            (a) =                                                        \
                (uint64_t)t_add_sub_x0 | ((uint64_t)t_add_sub_x1 << 32); \
            (b) =                                                        \
                (uint64_t)t_add_sub_y0 | ((uint64_t)t_add_sub_y1 << 32); \
        } while (0)
#else
#    define FPR_ADD_SUB(a, b, x, y)                  \
        do {                                         \
...
static inline f64 f64_sqr(f64 a)
{
    return f64_mul(a, a);
}
...
static inline f64 f64_inv(f64 a)
{
    return f64_div((f64){1.0}, a);
}

static inline f64 f64_half(f64 a)
{
...
size_t sign_core(unsigned logn, const int8_t *f, const int8_t *g,
                 ...
                 size_t ctx_len, const char *id, const uint8_t *hv,
                 size_t hv_len, const uint8_t *seed, size_t seed_len,
