...
static inline GAUSSIAN0_STORE *GAUSSIAN0_STORE_new(sampler_state *ss)
{
    ...
    int ret = posix_memalign((void **)&store, 64, sizeof(*store));
    if (ret != 0) {
        fprintf(stderr,
...
{
    return sampler_next_sse2(ss, _mm_load_sd((double *)&mu),
...
/* Input: 0 <= x < log(2)
   Output: trunc(x*2^63) */
TARGET_NEON
static inline int64_t mtwop63(float64x1_t x)
{
    const fpr_u twop63 = {FPR(4503599627370496, 11)};
 ...
 */
TARGET_NEON
static inline uint64_t expm_p63(float64x1_t x, float64x1_t ccs)
{
    /* The polynomial approximation of exp(-x) is from FACCT:
          https://eprint.iacr.org/2018/1234
       Specifically, the values are extracted from the implementation
...
/* Sample a bit with probability ccs*exp(-x) (for x >= 0). */
TARGET_NEON
static inline int ber_exp(sampler_state *ss, float64x1_t x,
                          float64x1_t ccs)
{
    const fpr_u LOG2_u = {LOG2};
    const fpr_u INV_LOG2_u = {INV_LOG2};
    ...
    int32_t si =
        (int32_t)vget_lane_s64(vcvt_s64_f64(vmul_f64(x, INV_LOG2_u.v)), 0);
    float64x1_t r =
        vsub_f64(x, vmul_f64(vcvt_f64_s64(vcreate_s64(si)), LOG2_u.v));
...
TARGET_NEON
static int32_t sampler_next_neon(sampler_state *ss, float64x1_t mu,
                                 float64x1_t isigma)
{
    const fpr_u HALF_u = {FPR(4503599627370496, -53)};
    const fpr_u INV_2SQRSIGMA0_u = {INV_2SQRSIGMA0};
...
int32_t sampler_next(sampler_state *ss, fpr mu, fpr isigma)
{
    return sampler_next_neon(ss, vld1_f64((const float64_t *)&mu),
...
/* Input: 0 <= x < log(2)
   Output: trunc(x*2^63) */
TARGET_NEON
static inline int64_t mtwop63(f64 x)
{
    const fpr_u twop63 = {FPR(4503599627370496, 11)};
    return f64_trunc(f64_mul(x, twop63.v));
}

static inline uint64_t expm_p63(f64 x, f64 ccs)
{
    /* The polynomial approximation of exp(-x) is from FACCT:
          https://eprint.iacr.org/2018/1234
       Specifically, the values are extracted from the implementation
...
/* Sample a bit with probability ccs*exp(-x) (for x >= 0). */
static inline int ber_exp(sampler_state *ss, f64 x, f64 ccs)
{
    const fpr_u LOG2_u = {LOG2};
    const fpr_u INV_LOG2_u = {INV_LOG2};
...
static int32_t sampler_next_rv64d(sampler_state *ss, f64 mu, f64 isigma)
{
    const fpr_u INV_2SQRSIGMA0_u = {INV_2SQRSIGMA0};
    const fpr_u SIGMA_MIN[] = {
        {FPR_ZERO},                   /* unused */
...
static inline uint64_t expm_p63(fpr x, fpr ccs)
{
    /* The polynomial approximation of exp(-x) is from FACCT:
          https://eprint.iacr.org/2018/1234
       Specifically, the values are extracted from the implementation
...
/* Sample a bit with probability ccs*exp(-x) (for x >= 0). */
static inline int ber_exp(sampler_state *ss, fpr x, fpr ccs)
{
    /* Reduce x modulo log(2): x = s*log(2) + r, with s an integer,
...
TARGET_SSE2 TARGET_NEON static void ffsamp_fft_inner(sampler_state *ss,
                                                     unsigned logn,
                                                     fpr *t0, fpr *t1,
                                                     fpr *g00, fpr *g01,
                                                     fpr *g11, fpr *tmp)
{
    const fpr_u INV_SIGMA[] = {
        {FPR_ZERO},                   /* unused */
    ...
    if (logn == 1) {
        ...
        __m128d cz =
            _mm_castsi128_pd(_mm_setr_epi32(0, 0, 0, -0x80000000));
        ...
        __m128d b = _mm_add_pd(_mm_shuffle_pd(b1, b2, 2),
        ...
        float64x1_t leaf =
            vmul_f64(vsqrt_f64(d11_re), INV_SIGMA[ss->logn].v);
        float64x1_t y0 =
            vcvt_f64_s64(vcreate_s64(sampler_next_neon(ss, w0, leaf)));
        float64x1_t y1 =
            vcvt_f64_s64(vcreate_s64(sampler_next_neon(ss, w1, leaf)));
        ...
        float64x2_t b1 =
            vmulq_f64(a, vreinterpretq_f64_u64(
                             veorq_u64(cz.x, vreinterpretq_u64_f64(l01))));
        ...
        leaf = vmul_f64(vsqrt_f64(d00_re), INV_SIGMA[ss->logn].v);
        x0 = vcvt_f64_s64(vcreate_s64(sampler_next_neon(ss, x0, leaf)));
        x1 = vcvt_f64_s64(vcreate_s64(sampler_next_neon(ss, x1, leaf)));
        ...
        f64 b_re = f64_sub(f64_mul(a_re, l01_re), f64_mul(a_im, l01_im));
        f64 b_im = f64_add(f64_mul(a_im, l01_re), f64_mul(a_re, l01_im));
        ...
        fpr zo_re =
            fpr_add(fpr_mul(mu_re, g01_re), fpr_mul(mu_im, g01_im));
        fpr d00_re = g00_re;
        fpr l01_re = mu_re;
    ...
    ffsamp_fft_inner(ss, logn - 1, w0, w1, right_00, right_01, right_11,
                     w2);
    ...
    ffsamp_fft_inner(ss, logn - 1, w0, w1, left_00, left_01, left_11, w2);
